...
def main(argv):
  ...
  linux_sysroot = (
      UpdateLinuxSysroot() if is_linux and not options.no_sysroot else None)

  out_dir = (
      options.out_path if options.out_path else os.path.join(REPO_ROOT, 'out'))
...
def WriteGNNinja(path, options, linux_sysroot):
  ...
  if is_posix:
    ...
    if is_linux and linux_sysroot != None:
