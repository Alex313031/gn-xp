...
def main(argv):
  ...
  args_list.add('--generate_compile_database',
                    action='store_true',
                    help=('Generate compile_commands.json under out dir ' +
                          'using `ninja -t compdb`.'))
...
def WriteGenericNinja(path, static_libraries, executables,
  ...
  if options.generate_compile_database:
    compdb = subprocess.check_output(['ninja', '-C', build_dir, '-t', 'compdb'], text=True)
