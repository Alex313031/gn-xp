...
void TestWithScope::SetupToolchain(Toolchain* toolchain) {
  ...
  // OBJC
  std::unique_ptr<Tool> objcxx_tool =
      std::make_unique<CTool>(Tool::TYPE_OBJCXX);
  SetCommandForTool(
  ...
  std::unique_ptr<Tool> solink_module =
      std::make_unique<CTool>(Tool::TYPE_SOLINK_MODULE);
  ...
  std::unique_ptr<Tool> copy_bundle_data_tool =
      std::make_unique<GeneralTool>(Tool::TYPE_COPY_BUNDLE_DATA);
  ...
  std::unique_ptr<Tool> compile_xcassets_tool =
      std::make_unique<GeneralTool>(Tool::TYPE_COMPILE_XCASSETS);
