...
void TestWithScope::SetupToolchain(Toolchain* toolchain) {
  ...
  rustc_tool->AsRust()->set_outputs(SubstitutionList::MakeForTest(
      "{{root_out_dir}}/{{rustc_output_prefix}}{{crate_name}}{{rustc_output_extension}}"));
  ...
  SetCommandForTool(
      ...
      "{{rustdeps}} {{externs}}",
      cdylib_tool.get());
  cdylib_tool->AsRust()->set_default_output_extension(".so");
  cdylib_tool->AsRust()->set_cdylib_output_extension(".so");
  cdylib_tool->AsRust()->set_outputs(SubstitutionList::MakeForTest(
      "{{root_output_dir}}/{{rustc_output_prefix}}{{crate_name}}{{rustc_output_extension}}"));
  ...
  SetCommandForTool(
      ...
      "{{rustdeps}} {{externs}}",
      dylib_tool.get());
  dylib_tool->AsRust()->set_default_output_extension(".so");
  dylib_tool->AsRust()->set_dylib_output_extension(".so");
  dylib_tool->AsRust()->set_outputs(SubstitutionList::MakeForTest(
      "{{root_output_dir}}/{{rustc_output_prefix}}{{crate_name}}{{rustc_output_extension}}"));
  toolchain->SetTool(std::move(dylib_tool));

  // PROC_MACRO
  std::unique_ptr<Tool> proc_macro_tool = Tool::CreateTool(RustTool::kRsToolProcMacro);
  SetCommandForTool(
      "{{rustenv}} rustc --crate-name {{crate_name}} {{source}} "
      "--crate-type {{crate_type}} {{rustflags}} -o {{output}} "
      "{{rustdeps}} {{externs}}",
      proc_macro_tool.get());
  proc_macro_tool->AsRust()->set_default_output_extension(".so");
  proc_macro_tool->AsRust()->set_proc_macro_output_extension(".so");
  proc_macro_tool->AsRust()->set_outputs(SubstitutionList::MakeForTest(
  ...
  SetCommandForTool(
      ...
      "{{rustdeps}} {{externs}}",
      rlib_tool.get());
  rlib_tool->AsRust()->set_outputs(SubstitutionList::MakeForTest(
      "{{target_out_dir}}/{{rustc_output_prefix}}{{crate_name}}{{rustc_output_extension}}"));
  ...
  SetCommandForTool(
      ...
      "{{rustdeps}} {{externs}}",
      staticlib_tool.get());
  staticlib_tool->AsRust()->set_default_output_extension(".a");
  staticlib_tool->AsRust()->set_staticlib_output_extension(".a");
  staticlib_tool->AsRust()->set_outputs(SubstitutionList::MakeForTest(
      "{{target_out_dir}}/{{rustc_output_prefix}}{{crate_name}}{{rustc_output_extension}}"));
