...
void AddSourceSetObjectFiles(const Target* source_set,
  ...
  if (source_set->config_values().has_precompiled_headers()) {
    if (used_types.Get(SOURCE_C)) {
      const CTool* tool = source_set->toolchain()->GetToolAsC(Tool::TYPE_CC);
    ...
    if (used_types.Get(SOURCE_CPP)) {
      const CTool* tool = source_set->toolchain()->GetToolAsC(Tool::TYPE_CXX);
      if (tool && tool->precompiled_header_type() == CTool::PCH_MSVC) {
    ...
    if (used_types.Get(SOURCE_M)) {
      const CTool* tool = source_set->toolchain()->GetToolAsC(Tool::TYPE_OBJC);
      if (tool && tool->precompiled_header_type() == CTool::PCH_MSVC) {
    ...
    if (used_types.Get(SOURCE_MM)) {
      const CTool* tool =
          source_set->toolchain()->GetToolAsC(Tool::TYPE_OBJCXX);
      if (tool && tool->precompiled_header_type() == CTool::PCH_MSVC) {
...
void NinjaCBinaryTargetWriter::WriteCompilerVars(
  ...
  if (used_types.Get(SOURCE_C)) {
    WriteOneFlag(target_, SUBSTITUTION_CFLAGS_C, has_precompiled_headers,
                 Tool::TYPE_CC, &ConfigValues::cflags_c, opts, path_output_,
                 out_);
  }
  if (used_types.Get(SOURCE_CPP)) {
    WriteOneFlag(target_, SUBSTITUTION_CFLAGS_CC, has_precompiled_headers,
                 Tool::TYPE_CXX, &ConfigValues::cflags_cc, opts, path_output_,
                 out_);
  }
  if (used_types.Get(SOURCE_M)) {
...
void NinjaCBinaryTargetWriter::WritePCHCommands(
  ...
  const CTool* tool_c = target_->toolchain()->GetToolAsC(Tool::TYPE_CC);
  ...
  const CTool* tool_cxx = target_->toolchain()->GetToolAsC(Tool::TYPE_CXX);
  ...
  const CTool* tool_objc = target_->toolchain()->GetToolAsC(Tool::TYPE_OBJC);
  ...
  const CTool* tool_objcxx =
      target_->toolchain()->GetToolAsC(Tool::TYPE_OBJCXX);
...
void NinjaCBinaryTargetWriter::WriteSources(
  ...
  for (const auto& source : target_->sources()) {
    ...
    if (tool_type != Tool::TYPE_NONE) {
      ...
      const CTool* tool = target_->toolchain()->GetToolAsC(tool_type);
...
void NinjaCBinaryTargetWriter::WriteCompilerBuildLine(
  ...
  out_ << ": " << rule_prefix_ << Tool::ToolTypeToName(tool_type);
  out_ << " ";
...
void NinjaCBinaryTargetWriter::WriteLinkerStuff(
  ...
  out_ << ": " << rule_prefix_
       << Tool::ToolTypeToName(Tool::GetToolTypeForTargetFinalOutput(target_));
