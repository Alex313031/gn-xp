...
bool EnsureFileIsGeneratedByDependency(const Target* target,
  ...
  if (check_data_deps || check_write) {
    check_write = false;
...
void Target::CheckSourceGenerated(const SourceFile& source) const {
  ...
  if (!EnsureFileIsGeneratedByDependency(this, out_file, true,
    ...
    // output check failed.
    consider_object_files = !check_data_deps;
    // Check that any data-written files are appropriately depended on.
    check_write = g_scheduler->IsFileGeneratedByWriteData(out_file);
    if (!EnsureFileIsGeneratedByDependency(
            this, out_file, true, consider_object_files, check_data_deps,
            check_write, &seen_targets))
...
bool Target::CollectMetadata(Err* err) {
  // This value doesn't appear anywhere in the source, since it's collected from
  // various places, and thus doesn't have parse node.
  write_data_ = Value(nullptr, Value::LIST);
  std::set<const Target*> targets_walked;
  const DepsIteratorRange& all_deps = GetDeps(Target::DEPS_ALL);
  for (const auto& dep : all_deps) {
    auto pair = targets_walked.insert(dep.ptr);
    if (pair.second) {
      if (!dep.ptr->GetMetadata(write_data_keys_, write_walk_keys_,
                                write_rebase_, &write_data_.list_value(),
                                &targets_walked, err))
        return false;
    }
  }
...
bool Target::WriteData(Err* err) {
  ...
  ConvertValueToOutput(settings(), write_data_, write_output_conversion_,
                       contents, err);
  if (err->has_error())
    return false;
  if (should_write_)
