...
bool Target::OnResolved(Err* err) {
  ...
  if (definition_scope_.type() == Value::SCOPE) {
    Value scope = definition_scope_.Resolve(this, err);
    if (err->has_error())
      return false;
    TargetGenerator::GenerateSpecificTarget(scope.scope_value(),
                                            defined_from(),
...
bool Target::WriteData(Err* err) {
  ...
  // data.
  if (contents_.type() == Value::NONE) {
    // Origin is set to the outputs location, so that errors with this value
    // get flagged on the right target.
    CHECK(action_values_.outputs().list().size() == 1U);
    contents_ = Value(action_values_.outputs().list()[0].origin(), Value::LIST);
    std::set<const Target*> targets_walked;
    if (!GetMetadata(data_keys_, walk_keys_, rebase_, /*deps_only = */ true,
                     &contents_.list_value(), &targets_walked, err))
      return false;
  }

  std::vector<SourceFile> outputs_as_sources;
  action_values_.GetOutputsAsSourceFiles(this, &outputs_as_sources);
  CHECK(outputs_as_sources.size() == 1U);
