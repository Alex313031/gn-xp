...
const char kMeta_Help[] =
  ...
  collecting metadata dependencies as specified by the givn walk key(s).
...
int RunMeta(const std::vector<std::string>& args) {
  ...
  std::vector<base::StringPiece> data_keys = base::SplitStringPiece(
      data_keys_str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
  if (data_keys.empty()) {
    return 1;
  }
  std::vector<base::StringPiece> walk_keys = base::SplitStringPiece(
      walk_keys_str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
  Err err;
  std::vector<Value> result =
      WalkMetadata(targets, data_keys, walk_keys, rebase_files, &err);
  if (err.has_error()) {
    err.PrintToStdout();
    return 1;
  }
  ...
  for (const auto& value : result)
    OutputString("  - " + value.ToString(false) + "\n");
  OutputString("\nExtracted from:\n", DECORATION_DIM);
  bool first = true;
