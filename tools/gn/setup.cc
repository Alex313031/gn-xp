...
std::u16string SysMultiByteTo16(base::StringPiece mb) {
  ...
  MultiByteToWideChar(CP_ACP, 0, mb.data(), mb_length,
                      base::ToWCharT(&wide[0]), charcount);
...
base::FilePath FindWindowsPython() {
  ...
  DWORD path_length = ::GetEnvironmentVariable(reinterpret_cast<LPCWSTR>(kPathEnvVarName), nullptr, 0);
  if (path_length == 0)
    return base::FilePath();
  std::unique_ptr<base::char16[]> full_path(new base::char16[path_length]);
  DWORD actual_path_length =
      ::GetEnvironmentVariable(reinterpret_cast<LPCWSTR>(kPathEnvVarName), reinterpret_cast<LPWSTR>(full_path.get()), path_length);
