...
bool Setup::FillArguments(const base::CommandLine& cmdline) {
  ...
  auto switchValue = cmdline.GetSwitchValueASCII(switches::kArgs);
  if (cmdline.HasSwitch(switches::kArgs) ||
     (gen_empty_args_ && !base::ReadFileToString(build_arg_file, &contents))) {
    if (!FillArgsFromCommandLine(
      switchValue == ""? kDefaultArgsGn: switchValue))
      return false;
    SaveArgsToFile();
    return true;
  }
