...
Value::Value() : type_(NONE), int_value_(0), origin_(nullptr) {}

Value::Value(const ParseNode* origin, Type t)
    : type_(t), origin_(origin) {
  switch (type_) {
    case BOOLEAN:
...
Value::Value(const Value& other)
  ...
  switch (type_) {
    ...
    case LIST:
      ...
      break;
    case SCOPE:
      new (&scope_value_) std::unique_ptr<Scope>(other.scope_value_.get()
          ? other.scope_value_->MakeClosure() : nullptr);
      break;
    case NONE:
      break;
    default:
...
Value::~Value() {
  using namespace std;
  switch (type_) {
    case STRING: string_value_.~string(); break;
    case LIST: list_value_.~vector<Value>(); break;
    case SCOPE: scope_value_.~unique_ptr<Scope>(); break;
    default:;
  }
}
