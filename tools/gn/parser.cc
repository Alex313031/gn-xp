...
void RenderToText(const base::Value& node, int indent_level,
  ...
  if (node.FindKey(kBeforeComment)) {
    for (auto& v : node.FindKey(kBeforeComment)->GetList()) {
      os << IndentFor(indent_level + 1) <<
          "+BEFORE_COMMENT(\"" << v.GetString() << "\")\n";
    }
  }
  if (node.FindKey(kSuffixComment)) {
    for (auto& v : node.FindKey(kSuffixComment)->GetList()) {
      os << IndentFor(indent_level + 1) <<
          "+SUFFIX_COMMENT(\"" << v.GetString() << "\")\n";
    }
  }
  if (node.FindKey(kAfterComment)) {
    for (auto& v : node.FindKey(kAfterComment)->GetList()) {
      os << IndentFor(indent_level + 1) <<
