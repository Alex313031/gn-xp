...
TEST_F(NinjaBuildWriterTest, SpaceInDepfile) {
  ...
  base::FilePath dependency(FILE_PATH_LITERAL("path with space/BUILD.gn"));
  base::FilePath dependency_realpath = base::MakeAbsoluteFilePath(dependency);
  g_scheduler->AddGenDependency(dependency_realpath);
