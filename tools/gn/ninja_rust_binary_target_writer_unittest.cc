...
TEST_F(NinjaRustBinaryTargetWriterTest, RustSourceSet) {
  ...
  target.source_types_used().Set(SOURCE_RS);
...
TEST_F(NinjaRustBinaryTargetWriterTest, RustExecutable) {
  ...
  source_set.source_types_used().Set(SOURCE_RS);
  ...
  target.sources().push_back(SourceFile("//foo/main.rs"));
  target.source_types_used().Set(SOURCE_RS);
  target.rust_values().set_crate_root(&target.sources().back());
...
TEST_F(NinjaRustBinaryTargetWriterTest, RlibDeps) {
  ...
  rlib.sources().push_back(SourceFile("//foo/mylib.rs"));
  rlib.sources().push_back(SourceFile("//foo/lib.rs"));
  rlib.source_types_used().Set(SOURCE_RS);
  rlib.rust_values().set_crate_root(&rlib.sources().back());
...

  {
    ...
    const char expected[] =
        ...
        "\n"
        "build obj/foo/libmylib.rlib: rustc ../../foo/lib.rs | "
        "../../foo/mylib.rs ../../foo/lib.rs\n";
    std::string out_str = out.str();
    EXPECT_EQ(expected, out_str) << out_str;
  }
  ...
  another_rlib.sources().push_back(SourceFile("//foo/lib.rs"));
  another_rlib.source_types_used().Set(SOURCE_RS);
  another_rlib.rust_values().set_crate_root(&another_rlib.sources().back());
...

  {
    ...
    const char expected[] =
        ...
        "  rlibs = --extern direct=obj/foo/libdirect.rlib\n"
        "  rustdeps = -Ldependency=obj/foo -Ldependency=obj/foo\n";
    std::string out_str = out.str();
