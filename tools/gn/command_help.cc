...
void PrintToplevelHelp() {
  ...
  PrintShortHelp(
      "output_conversion: Specifies how to transform a variable to output.");
...
void PrintAllHelp() {
  ...
  PrintLongHelp(kOutputConversion_Help, "output_conversion");
...
int RunHelp(const std::vector<std::string>& args) {
  ...
  random_topics["output_conversion"] = []() {
    PrintLongHelp(kOutputConversion_Help);
  };
