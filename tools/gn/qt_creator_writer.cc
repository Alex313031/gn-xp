...
const std::map<std::string, CVersion> flagToCVersion {
  {"-std=gnu99" , CVersion::C99},
  {"-std=c99"   , CVersion::C99},
  {"-std=gnu11" , CVersion::C11},
  {"-std=c11"   , CVersion::C11}
};
const std::map<std::string, CxxVersion> flagToCxxVersion {
  {"-std=gnu++11", CxxVersion::CXX11}, {"-std=c++11", CxxVersion::CXX11},
  {"-std=gnu++98", CxxVersion::CXX98}, {"-std=c++98", CxxVersion::CXX98},
  {"-std=gnu++03", CxxVersion::CXX98}, {"-std=c++03", CxxVersion::CXX98},
  {"-std=gnu++14", CxxVersion::CXX14}, {"-std=c++14", CxxVersion::CXX14}, {"-std=c++1y", CxxVersion::CXX14},
  {"-std=gnu++17", CxxVersion::CXX17}, {"-std=c++17", CxxVersion::CXX17}, {"-std=c++1z", CxxVersion::CXX17},
};

template<typename Enum>
struct CompVersion
{
  bool operator()(Enum a, Enum b) { return static_cast<int>(a) < static_cast<int>(b); }
};
...
void ParseCompilerOption(const std::string& flag, CompilerOptions* options) {
  auto c_ver = flagToCVersion.find(flag);
  if (c_ver != flagToCVersion.end())
    options->SetCVersion(c_ver->second);

  auto cxx_ver = flagToCxxVersion.find(flag);
  if (cxx_ver != flagToCxxVersion.end())
...
void QtCreatorWriter::HandleTarget(const Target* target) {
  ...
  for (ConfigValuesIterator it(target); !it.done(); it.Next()) {
    ...
    auto add_define_version = [&] (auto &ver) {
      if (ver)
