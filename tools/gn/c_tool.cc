...
CTool::CTool(Tool::ToolType t)
    : Tool(t), depsformat_(DEPS_GCC), precompiled_header_type_(PCH_NONE) {}
...
bool CTool::ValidateRuntimeOutputs(Err* err) {
  ...
  if (type_ != Tool::TYPE_SOLINK && type_ != Tool::TYPE_SOLINK_MODULE &&
      type_ != Tool::TYPE_LINK) {
...
bool CTool::ValidateLinkAndDependOutput(const SubstitutionPattern& pattern,
  ...
  if (type_ != TYPE_SOLINK && type_ != TYPE_SOLINK_MODULE) {
...
bool CTool::InitTool(Scope* scope, Toolchain* toolchain, Err* err) {
  ...
  Tool::InitTool(scope, toolchain, err);
