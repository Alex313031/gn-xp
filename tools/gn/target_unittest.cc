...
TEST(TargetTest, CollectMetadataNoRecurse) {
  ...
  one.GetMetadata(data_keys, walk_keys, false, false, &result, &targets, &err);
...
TEST_F(TargetTest, WriteMetadataCollection) {
  ...
  TestTarget generator(setup, "//foo:write", Target::WRITE_DATA);
  generator.action_values().outputs() =
      SubstitutionList::MakeForTest("//out/Debug/metadata.json");
  generator.set_should_write(false);
  g_scheduler->AddWriteDataTarget(&generator);
