...
bool IsLinkerTool(Tool::ToolType type) {
  ...
  return type == Tool::TYPE_SOLINK ||
         type == Tool::TYPE_SOLINK_MODULE || type == Tool::TYPE_LINK;
}
...
bool ValidateLinkAndDependOutput(const Tool* tool,
  ...
  if (tool_type != Tool::TYPE_SOLINK &&
      tool_type != Tool::TYPE_SOLINK_MODULE) {
    *err = Err(tool->defined_from(),
...
Value RunTool(Scope* scope,
  ...
  Tool::ToolType tool_type = Toolchain::ToolNameToType(tool_name);
