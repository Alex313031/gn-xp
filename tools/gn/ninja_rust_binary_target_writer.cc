...
void WriteCrateVars(const Target* target,
                    ...
                    EscapeOptions opts,
                    std::ostream& out) {
  WriteVar(RustSubstitutionCrateName.ninja_name,
  ...
  WriteVar(RustSubstitutionCrateType.ninja_name, crate_type, opts, out);

  if (!target->output_extension_set()) {
    DCHECK(tool->AsRust());
    WriteVar(RustSubstitutionOutputExtension.ninja_name,
             tool->AsRust()->rustc_output_extension(
                 target->output_type(), target->rust_values().crate_type()),
             opts, out);
  } else if (target->output_extension().empty()) {
    WriteVar(RustSubstitutionOutputExtension.ninja_name, "", opts, out);
  } else {
    WriteVar(RustSubstitutionOutputExtension.ninja_name,
  ...
  if (target->output_type() == Target::RUST_LIBRARY ||
    ...
    WriteVar(RustSubstitutionOutputPrefix.ninja_name, "lib", opts, out);
}  // namespace
...
void NinjaRustBinaryTargetWriter::WriteCompilerVars() {
  ...
  WriteOneFlag(target_, &RustSubstitutionRustFlags, false,
               RustTool::kRsToolRustc, &ConfigValues::rustflags, opts,
               path_output_, out_);

  WriteOneFlag(target_, &RustSubstitutionRustEnv, false, RustTool::kRsToolRustc,
               &ConfigValues::rustenv, opts, path_output_, out_);

  WriteSharedVars(subst);
}

void NinjaRustBinaryTargetWriter::WriteRlibs() {
  std::vector<const Target*> rlibs;
  for (const auto& pair : target_->GetDeps(Target::DEPS_LINKED)) {
    if (pair.ptr->output_type() == Target::RUST_LIBRARY) {
      rlibs.push_back(pair.ptr);
    }
  }
  if (rlibs.empty())
    return;
  out_ << "  rlibs =";
  for (const Target* rlib : rlibs) {
    out_ << " --extern ";
    out_ << std::string(rlib->rust_values().crate_name()) << "=";
    path_output_.WriteFile(out_, rlib->dependency_output_file());
  }
  out_ << std::endl;
}
