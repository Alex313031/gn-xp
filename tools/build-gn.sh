...
JEMALLOC_RELEASE=
JEMALLOC_SOURCE_URL=
JEMALLOC_BASE_URL=https://github.com/jemalloc/jemalloc/releases/download/
DEFAULT_JEMALLOC_RELEASE=5.3.0
...
for OPT; do
  case "${OPT}" in
    ...
    --rpmalloc-revision=*)
      ...
      ;;
    --jemalloc-release=*)
      JEMALLOC_RELEASE="${OPT#--jemalloc-release=}"
      if [[ -z "${JEMALLOC_SOURCE_URL}" ]]; then
        JEMALLOC_SOURCE_URL="${JEMALLOC_BASE_URL}/${JEMALLOC_RELEASE}/jemalloc-${JEMALLOC_RELEASE}.tar.bz2"
      fi
      ;;
    --jemalloc-source-url=*)
      JEMALLOC_SOURCE_URL="${OPT#--jemalloc-source-url=}"
      ;;
    --debug)
      DEBUG=true
      ;;
...
Valid options:
  ...
  --jemalloc-release=VERSION
                         jemalloc release number [$DEFAULT_JEMALLOC_RELEASE]
  --jemalloc-source-url=URL
                         Download URL base for jemalloc source archive.
                         [${JEMALLOC_BASE_URL}/\$JEMALLOC_RELEASE/jemalloc-\$JEMALLOC_RELEASE.tar.bz2]
...
download_and_build_rpmalloc () {
  ...
  (
    ...
    ./configure.py $RPMALLOC_CONFIG_FLAGS
...
download_and_build_jemalloc () {
  ...
  if [[ -z "$JEMALLOC_RELEASE" ]]; then
    JEMALLOC_RELEASE=$DEFAULT_JEMALLOC_RELEASE
  fi
  if [[ -z "$JEMALLOC_SOURCE_URL" ]]; then
    JEMALLOC_SOURCE_URL="${JEMALLOC_BASE_URL}/${JEMALLOC_RELEASE}/jemalloc-${JEMALLOC_RELEASE}.tar.bz2"
  fi
  wget -q -O - "$JEMALLOC_SOURCE_URL" | tar xjf - --strip-components=1 -C "$JEMALLOC_SRC"
  (
    cd "${JEMALLOC_SRC}"
    ...
    ./configure \
      ...
      --disable-libdl \
      --disable-syscall \
      --disable-stats > $JEMALLOC_LOG 2>&1
    make -j$(nproc) >> $JEMALLOC_LOG 2>&1
  )
