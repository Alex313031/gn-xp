...
class ConfigInserter : public ConstNodeVisitor {
 public:
  ...
  bool ActOn(const FunctionCallNode &node) override {
    ...
    if (args->contents().size() != 2)
      return true;

    const auto *block = args->contents()[1]->AsBlock();
    if (!block)
      return true;

    // Search for sources list.
    if (!std::any_of(block->statements().begin(), block->statements().end(),
                     [this](const std::unique_ptr<ParseNode> &stmt){
                       return isSourcesListStmt(*stmt);
                     }))
      return true;
    //for (const auto &stmt : block->statements()) {
    //  if (isSourcesListStmt(*stmt)) {
    //    // Found it
    //  }
    //}
    return false;
 ...
 private:
  // sources = [...]
  bool isSourcesListStmt(const ParseNode &node) {
...
int main(int argc, char **argv) {
  ...
  std::unique_ptr<Value> json_args;
