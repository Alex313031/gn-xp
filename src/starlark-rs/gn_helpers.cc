...
std::unique_ptr<GNValueResult> GNExecContext::GetValue(rust::Str ident, bool counts_as_used) const {
  const Value* value = scope_->GetValue(std::string_view(ident.data(), ident.length()), counts_as_used);
  return std::make_unique<GNValueResult>(value);
}
