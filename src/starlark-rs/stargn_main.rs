...
mod ffi2 {
    ...
    unsafe extern "C++" {
        ...
        type Value; // autocxx can't handle this due to ambiguous namespace with base::Value
        type Value_Type;
        #[cxx_name = "type"] fn get_type(self: &Value) -> Value_Type;
        ...
        type GNValueResult; // autocxx can't handle this due to transitive issue with Value
        fn has_value(self: &GNValueResult) -> bool;
        fn get_value(self: &GNValueResult) -> &Value;
        ...
        fn GetValue(self: &GNExecContext, ident: &str, counts_as_used: bool) -> UniquePtr<GNValueResult>;
    }
...
fn gn_module(builder: &mut GlobalsBuilder) {
    #[starlark_module]
    fn gn_members(builder: &mut GlobalsBuilder) {
        fn defined<'v>(
            #[starlark(require = pos)] name: &str,
            eval: &mut Evaluator<'v, '_>,
        ) -> anyhow::Result<Value<'v>> {
            let context = EvalContext::from_starlark(eval)?;
            let maybe_value = context.gn_exec_context.borrow().GetValue(name, false);
            Ok(Value::new_bool(maybe_value.has_value()))
        }
        fn value<'v>(
            #[starlark(require = pos)] name: &str,
            eval: &mut Evaluator<'v, '_>,
            ...
            let context = EvalContext::from_starlark(eval)?;
            let maybe_value = context.gn_exec_context.borrow().GetValue(name, true);
            if maybe_value.has_value() {
                let value = maybe_value.get_value();
                match value.get_type() {
                    ffi2::Value_Type::BOOLEAN => return Ok(Value::new_bool(*value.boolean_value())),
                    _ => bail!("can't handle this type of variable yet")
                }
            }
            bail!("Undefined identifier")
        }
    }
    builder.struct_("gn", gn_members);
}

#[starlark_module]
fn global_module(builder: &mut GlobalsBuilder) {
    fn component<'v>(
        #[starlark(kwargs)] kwargs: DictRef<'v>,
        eval: &mut Evaluator<'v, '_>,
    ) -> anyhow::Result<NoneType> {
        run_gn_function("component", &kwargs, EvalContext::from_starlark(eval)?)
    }
    fn executable<'v>(
        #[starlark(kwargs)] kwargs: DictRef<'v>,
        eval: &mut Evaluator<'v, '_>,
    ...
    fn source_set<'v>(
        ...
        run_gn_function("source_set", &kwargs, EvalContext::from_starlark(eval)?)
    }
}
...
fn parse_starlark(gn_input_file: &ffi::InputFile, filename: &CxxString) -> anyhow::Result<Box<ParseResult>> {
    let dialect = Dialect {enable_top_level_stmt: true, ..Dialect::Standard};
