...
#include "cxxgen.h"
#include "cxxgen1.h"
#include "starlark-rs/starlark_loader.h"
...
int call_main(int argc, char** argv) {
  ...
  // If this is gen, hook Starlark loader. Otherwise defer to normal GN behavior.
  int retval;
  if (!args.empty() && args[0] == "gen") {
    args.erase(args.begin());
    MsgLoop msg_loop;
    retval = Gen::RunWithAlternateLoader([](Setup* setup) -> std::unique_ptr<InputAlternateLoader> {
        return std::make_unique<StarlarkInputLoader>(&setup->build_settings());
    }, args);
  } else {
    std::string command;
