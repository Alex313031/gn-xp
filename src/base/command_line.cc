...
// static
std::string CommandLine::StringTypeToUTF8(const StringPieceType& input) {
  return WideToUTF8(input);
}

// static
CommandLine::StringType CommandLine::UTF8ToStringType(StringPiece input) {
...
std::vector<CommandLine::StringType> CommandLine::GetSwitchValuesNative(
  ...
  auto iter = switches_.lower_bound(switch_string);
  while (iter != switches_.end() && iter->first == switch_string) {
