  ...
  if (out_toolchain) {
    ...
    if (toolchain_piece.empty()) {
      *out_toolchain = current_toolchain;
      ...
      if (!Resolve(current_dir, source_root, current_toolchain,
                   original_value, toolchain_piece,
                   &toolchain_dir, &toolchain_name, nullptr, err)) {
        return false;
      }
...
Label Label::Resolve(const SourceDir& current_dir,
  ...
  if (!::Resolve(current_dir, source_root, current_toolchain, input,
                 input_string, &ret.dir_, &ret.name_, &ret.toolchain_,
                 err))
    return Label();
  return ret;
}
...
std::string Label::GetUserVisibleName(bool include_toolchain) const {
  ...
  ret.reserve(dir_.value().size() + name_.str().size() + 1 + (include_toolchain ? toolchain_.str().size() + 2 : 0));
