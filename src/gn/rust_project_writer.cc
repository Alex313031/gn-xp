...
// AddTarget(build_settings, target, lookup, sysroot_lookup,
//           crate_list);
...
void WriteCrates(const BuildSettings* build_settings,
  ...
  for (auto& crate : crate_list) {
    ...
    rust_project << NEWLINE "    \"" << FilePathToUTF8(root_dir) << "\"";
  }
  ...
  for (auto& crate : crate_list) {
    ...
    base::FilePath crate_root = build_settings->GetFullPath(crate.root());

    rust_project << NEWLINE "    {" NEWLINE;
    rust_project << "      \"crate_id\": " << crate.index() << "," NEWLINE;
    rust_project << "      \"root_module\": \"" << FilePathToUTF8(crate_root)
                 << "\"," NEWLINE;
    rust_project << "      \"label\": \"" << crate.label() << "\"," NEWLINE;
    rust_project << "      \"deps\": [";
