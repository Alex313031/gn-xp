...
std::optional<std::string> FindArgValue(const char* arg,
  ...
  for (auto current = args.begin(); current != args.end();) {
    ...
    // if the previous value wasn't "arg", and after the above increment the end
    // hasn't been reached, then this is the tuple value.
...
void AddTarget(const BuildSettings* build_settings,
  ...
  ConfigList cfgs = FindAllArgValuesAfterPrefix(std::string("--cfg="), compiler_args);
