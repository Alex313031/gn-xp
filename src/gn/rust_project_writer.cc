...
void WriteDeps(const Target* target,
  ...
  for (const auto& dep : target->rust_values().transitive_libs().GetOrdered()) {
    ...
    rust_project << NEWLINE
                 ...
                 << "          \"name\": \"" << dep->rust_values().crate_name() << "\"" NEWLINE
                 << "        }";
    first = false;
  }
...
// TODO parse sysroot structure from toml files. This is fragile and might break if upstream
...
void AddTarget(const Target* target,
  ...
  rust_project << "      \"key_value_cfgs\": {" NEWLINE;
  bool first_kv = true;
