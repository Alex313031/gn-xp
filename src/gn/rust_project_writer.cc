...
void GetRustDeps(const Target* target, TargetsVec* rust_deps) {
  for (const auto& pair : target->GetDeps(Target::DEPS_LINKED)) {
    ...
    const auto output_type = dep->output_type();

    if (output_type == Target::OutputType::RUST_LIBRARY ||
        output_type == Target::OutputType::RUST_PROC_MACRO) {
      rust_deps->push_back(dep);
    } else if (output_type == Target::OutputType::GROUP) {
      // we must go deeper
