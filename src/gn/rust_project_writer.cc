...
bool RustProjectWriter::RunAndWriteFiles(const BuildSettings* build_settings,
  ...
  RenderJSON(build_settings, all_targets, rust_sysroot, json);

  return true;
}

using TargetIdxMap = std::unordered_map<const Target*, uint32_t>;
using SysrootIdxMap = std::unordered_map<std::string_view, uint32_t>;
...
void WriteDeps(const Target* target,
  ...
  auto std_idx = sysroot_lookup.find("std");
  if (std_idx != sysroot_lookup.end()) {
    if (!first)
      rust_project << ",";
...
void AddSysrootCrate(const std::string_view crate,
  ...
  if (crate == "std") {
    for (auto dep : std_deps) {
      AddSysrootCrate(dep, rust_sysroot, count, sysroot_lookup, rust_project,
                      first);
      first = false;
    }
  }
  ...
  auto crate_path = FilePathToUTF8(rust_sysroot) +
                    "/lib/rustlib/src/rust/src/lib" + std::string(crate) +
                    "/lib.rs";
  rust_project << "    {" NEWLINE;
...
void RustProjectWriter::RenderJSON(const BuildSettings* build_settings,
  ...
  // Sysroot crates
  base::FilePath empty(FILE_PATH_LITERAL(""));
  if (rust_sysroot != empty) {
    for (const auto& crate : sysroot_crates) {
      AddSysrootCrate(crate, rust_sysroot, &count, sysroot_lookup, rust_project,
                      first);
      first = false;
    }
  }
