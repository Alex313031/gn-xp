...
std::string FlagsGetter(
    const Target* target,
    const std::vector<T, std::allocator<T>>& (ConfigValues::*getter)() const,
    const Writer& writer) {
  std::string result;
...
void SetupCompileFlags(const Target* target,
  ...
  flags.defines = FlagsGetter(target, &ConfigValues::defines,
                              DefineWriter(ESCAPE_SPACE, true));

  flags.framework_dirs = FlagsGetter(target, &ConfigValues::framework_dirs,
                                     FrameworkDirsWriter(path_output, "-F"));

  flags.frameworks =
      FlagsGetter(target, &ConfigValues::frameworks,
                  FrameworksWriter(ESCAPE_SPACE, true, "-framework"));

  flags.includes = FlagsGetter(target, &ConfigValues::include_dirs,
