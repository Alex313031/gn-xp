...
bool CompileCommandsWriter::RunAndWriteFiles(
  ...
  for (auto& target :
    ...
    std::vector<std::string> target_split = base::SplitString(target, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
std::vector<const Target*> CompileCommandsWriter::FilterTargets(
  ...
  for (auto& target : all_targets) {
    bool match = false;
    for(auto& target_filter_label : target_filters_set) {
      if(target_filter_label.dir().is_null() || target_filter_label.dir().value() == target->label().dir().value()) {
