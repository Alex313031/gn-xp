...
TEST_F(TargetTest, PublicConfigs) {
  ...
  const auto& dep_on_pub_all_libs = resolved.all_libs(&dep_on_pub);
...
TEST_F(TargetTest, SharedInheritance) {
  ...
  // inherited libraries.
  const auto& inter_inherited = resolved.inherited_libraries(&inter);
  ...
  const auto& exe_inherited = resolved.inherited_libraries(&exe);
