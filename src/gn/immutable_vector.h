...
class ImmutableVector : public ImmutableVectorView<T> {
 public:
  ...
  template <typename C,
            ...
            typename = std::void_t<decltype(std::declval<C>().size())>>
  ...
  ~ImmutableVector() {
    if (this->header_) {
      ...
      base::aligned_free(this->header_);
    }
  }
 ...
 protected:
  ...
  static Header* AllocateFor(size_t count) {
    ...
    auto* header = reinterpret_cast<Header*>(base::aligned_alloc(
        alignof(T), sizeof(Header) + (count - 1u) * sizeof(T)));
    header->size = count;
    return header;
  }
