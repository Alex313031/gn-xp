...
void NinjaActionTargetWriter::Run() {
  ...
  for (const auto& pair : target_->GetDeps(Target::DEPS_LINKED)) {
    if (pair.ptr->IsDataOnly()) {
      data_outs.push_back(pair.ptr->dependency_output());
    } else {
  ...
  for (const auto& dep : target_->data_deps()) {
    if (dep.ptr->has_dependency_output())
      data_outs.push_back(dep.ptr->dependency_output());
  }
  WriteStampOrPhonyForTarget(output_files, data_outs);
}
