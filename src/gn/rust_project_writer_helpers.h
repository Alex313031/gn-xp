...
using CrateIdx = size_t;

using ConfigList = std::vector<std::string>;
using Dependency = std::pair<CrateIdx, std::string>;
...
class Crate {
 public:
  Crate(SourceFile root, CrateIdx index, std::string label, std::string edition)
      : root_(root), index_(index), label_(label), edition_(edition) {}

  ~Crate() = default;

  // Add a config item to the crate.
  void AddConfigItem(std::string cfg_item) { cfgs_.push_back(cfg_item); }

  // Add another crate as a dependency of this one.
  void AddDependency(CrateIdx index, std::string name) {
  ...
  ConfigList& configs() { return cfgs_; }
...
using SysrootCrateIdxMap = std::unordered_map<std::string_view, CrateIdx>;
