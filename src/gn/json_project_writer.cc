...
StringOutputBuffer JSONProjectWriter::GenerateJSON(
  ...
  std::map<std::string, const Toolchain*> toolchains;
  json_writer.BeginDict("targets");
  {
    for (const auto* target : sorted_targets) {
      ...
      if (target->toolchain()) {
        toolchains[target->toolchain()->label().GetUserVisibleName(false)] = target->toolchain();
      }
    }
  }
  ...
  json_writer.BeginDict("toolchains");
  {
    for (const auto &tool_chain_kv : toolchains) {
      ...
      for (const auto &tool_kv : tools) {
        base::Value toolInfo{base::Value::Type::DICTIONARY};
        auto setIfNotEmptry = [&](const auto &key, const auto &value) {
          if (value.size())
            toolInfo.SetKey(key, base::Value{value});
        };
        auto setSubstitutionList = [&](const auto &key, const SubstitutionList &list) {
          if (list.list().empty())
            return;
          base::Value values{base::Value::Type::LIST};
          for (const auto &v: list.list())
            values.GetList().emplace_back(base::Value{v.AsString()});
          toolInfo.SetKey(key, std::move(values));
        };
        ...
        setIfNotEmptry("default_output_extension", tool->default_output_extension());
        setIfNotEmptry("default_output_dir", tool->default_output_dir().AsString());
        ...
        toolchain.SetKey(tool_kv.first, std::move(toolInfo));
      }
      std::string json_dict;
      base::JSONWriter::WriteWithOptions(toolchain,
                                         base::JSONWriter::OPTIONS_PRETTY_PRINT,
                                         &json_dict);
      json_writer.AddJSONDict(tool_chain_kv.first, json_dict);
    }
  }
