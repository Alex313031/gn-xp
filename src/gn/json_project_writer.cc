...
std::string JSONProjectWriter::RenderJSON(
  ...
  const auto &build_path = build_settings->root_path().NormalizePathSeparatorsTo('/');
  for (const auto& other_file : fileset) {
    std::string file;
    if (MakeAbsolutePathRelativeIfPossible(build_path.value(), other_file.NormalizePathSeparatorsTo('/').value(), &file))
      inputs.Append(std::make_unique<base::Value>(std::move(file)));
  }
  settings->SetKey("input_files", std::move(inputs));
