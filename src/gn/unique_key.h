...
//     // find() method.
//     //
//     using FastStringSet = std::set<UniqueKey, UniqueKey::FastCompare>
//
...
//     using FastStringMap = std::map<UniqueKey, VALUE, UniqueKey::FastCompare>
//
class UniqueKey {
 public:
  ...
  // E.g.:
  //    using FastSet = std::unordered_set<UniqueKey, FastHash, FastEqual>;
  //    using FastMap = std::map<UniqueKey, Value, FastCompare>;
  //
  // IMPORTANT: Note that FastMap above is ordered based in the UniqueKey
  //            pointer value, not the string content. If you want to get
  //
  struct FastHash {
