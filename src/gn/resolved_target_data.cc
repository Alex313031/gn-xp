...
class TargetInfoMap {
 public:
  ...
  bool Contains(const Target* target) { return targets_.Contains(target); }

  TargetInfo* GetInfo(const Target* target) {
  ...
  std::vector<std::unique_ptr<TargetInfo>> infos_;
};

// Hashing struct type used by TargetPublicPairListBuilder to ensure that
// only the target pointer is used during lookups.
struct TargetPublicPairHash {
  size_t operator()(TargetPublicPair p) const noexcept {
    return std::hash<const Target*>()(p.target());
  }
};

// Equality struct type used by TargetPublicPairListBuilder to ensure that
// only the target pointer is used during lookups.
struct TargetPublicPairEqualTo {
...
// pointers are unique. Used internally to compute inherited libraries for
// all binaries, as well as rust-specific targets.
class TargetPublicPairListBuilder
    : public UniqueVector<TargetPublicPair,
                          TargetPublicPairHash,
                          TargetPublicPairEqualTo> {
 public:
  ...
  void Append(const Target* target, bool is_public) {
    ...
    if (!ret.first && is_public) {
      // Modify the |is_public| flag in-place. This works because
      // the hash and equal_to template type parameters ignore the flag.
...
class ResolvedTargetData::Impl {
 public:
  ...
  TargetSet recursive_hard_deps(const Target* target) const {
    TargetInfo* info = map_.GetInfo(target);
