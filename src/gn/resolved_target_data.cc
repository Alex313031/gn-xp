...
struct TargetInfo {
  ...
  std::vector<const Target*> hard_deps;
...
class ResolvedTargetData::Impl {
 public:
  ...
  base::span<const TargetPublicPair> inherited_libraries(
      const Target* target) const {
  ...
  base::span<const TargetPublicPair> rust_transitive_inherited_libs(
      const Target* target) const {
 ...
 private:
  ...
  void ComputeLibInfo(TargetInfo* info) const {
    ...
    info->lib_dirs = std::vector<SourceDir>(all_lib_dirs.release());
    info->libs = std::vector<LibFile>(all_libs.release());
  ...
  void ComputeHardDeps(TargetInfo* info) const {
    ...
    info->hard_deps = all_hard_deps.ToVector();
...
base::span<const SourceDir> ResolvedTargetData::all_lib_dirs(
    const Target* target) const {
  return GetImpl()->all_lib_dirs(target);
}

base::span<const LibFile> ResolvedTargetData::all_libs(
    const Target* target) const {
...
base::span<const SourceDir> ResolvedTargetData::all_framework_dirs(
    const Target* target) const {
  return GetImpl()->all_framework_dirs(target);
}

base::span<const std::string> ResolvedTargetData::all_frameworks(
    const Target* target) const {
  return GetImpl()->all_frameworks(target);
}

base::span<const std::string> ResolvedTargetData::all_weak_frameworks(
    const Target* target) const {
