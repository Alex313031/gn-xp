...
class UniqueVectorHashSet : public UniqueVectorHashTableBase {
 public:
  ...
  template <typename T, typename EQUAL_TO = std::equal_to<T>>
  Node* Lookup(size_t hash, const T& item, const std::vector<T>& vector) const {
    ...
    return BaseType::NodeLookup(hash32, [&](const Node* node) {
      return hash32 == node->hash32 && EQUAL_TO()(vector[node->index()], item);
    });
  }
...
template <typename T,
          ...
          typename EQUAL_TO = std::equal_to<T>>
class UniqueVector {
 public:
  ...
  // |index| is the item's index in the set and cannot be kIndexNone.
  // This can be used to implement a map using a UniqueVector<>
  // for keys, and a parallel array for values.
  ...
  // |index| is the item's index in the vector and cannot be
  // kIndexNone.
  template <typename... ARGS>
 ...
 private:
  ...
  UniqueVectorNode* Lookup(const T& t, size_t* hash) const {
    *hash = HASH()(t);
    return set_.Lookup<T, EQUAL_TO>(*hash, t, vector_);
  }
