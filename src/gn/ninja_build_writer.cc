...
Phony rules
  ...
  so are generated with the following priority:
    ...
    8. Full GN labels for targets in the default toolchain, for example
       "//tools/fruit:doom_melon" (including the leading slashes).

    9. Fully qualified labels for all targets in all toolchains so you can use
       build using the same name as GN uses like "ninja
       '//foo/bar:test(//build/toolchain:host)'". This is most useful for
       machine execution of ninja or copy-paste from GN label queries, as the
       names can be quite verbose.
...
bool NinjaBuildWriter::WritePhonyAndAllRules(Err* err) {
  ...
  // Write phony rules for default toolchain targets without the toolchain.
  for (const Target* target : default_toolchain_targets_) {
    const Label& label = target->label();
    // Something like "//foo/bar:baz" (no toolchain, but fully qualified).
    std::string long_name = label.GetUserVisibleName(false);
    if (written_rules.insert(long_name).second)
      WritePhonyRule(target, long_name);
  }

  // Write phony rules for *all* targets, fully qualified using "(toolchain)"
  // syntax.
  for (const Target* target : all_targets_) {
    const Label& label = target->label();
    // Something like "//foo/bar:baz(//build/toolchain:zip)", unmodified.
    std::string long_name = label.GetUserVisibleName(true);
    if (written_rules.insert(long_name).second)
      WritePhonyRule(target, long_name);
  }
