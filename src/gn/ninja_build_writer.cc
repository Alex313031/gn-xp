...
Phony rules
  ...
  so are generated with the following priority:
    ...
    8. Full label name with no leading slashes, and including the toolchain.
       So you can use "ninja 'foo/bar:test(//build/toolchain:host)'" to build
       "//foo/bar:test(//build/toolchain:host)".
...
bool NinjaBuildWriter::WritePhonyAndAllRules(Err* err) {
  ...
  for (const Target* target : all_targets_) {
    ...
    // Write the long name "foo/bar:baz(//build/toolchain:zip)" for the target
    // "//foo/bar:baz(//build/toolchain:zip)".
