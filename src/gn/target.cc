...
bool Target::CheckForRealInputs() {
  ...
  CHECK(dependency_output_phony_);

  // TODO(194): This method is currently just checking the relevant inputs for
  // the current list of output types that result in phony targets. As the list
  // of phony targets expands, this method should be updated to properly account
  // for which inputs matter for the given output type.
...
bool Target::FillOutputFiles(Err* err) {
  ...
  switch (output_type_) {
    case SOURCE_SET: {
      ...
      dependency_output_phony_.value().append(".phony");

      if (CheckForRealInputs()) {
        dependency_output_file_ = dependency_output_phony_;
      }
      break;
    }
    case GROUP:
