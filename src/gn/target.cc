    ...
    *err =
        Err(defined_from(), "This target uses an undefined tool.",
            base::StringPrintf(
                ...
                Tool::GetToolTypeForTargetFinalOutput(
                    this, /*replace_stamp_with_phony=*/ false)));
  }
  return false;
}
...
bool Target::FillOutputFiles(Err* err) {
  ...
  switch (output_type_) {
    ...
    case GROUP:
    case SOURCE_SET: {
      if (settings()->build_settings()->replace_stamp_with_phony()) {
        if (HasRealInputs()) {
          dependency_output_phony_ =
              GetBuildDirForTargetAsOutputFile(this, BuildDirType::PHONY);
          dependency_output_phony_->value().append(label().name());
        }
      } else {
