...
class JsonBuilder {
 public:
  JsonBuilder(const BuildSettings* build_settings, const Builder& builder)
      : build_settings_{build_settings}, builder_{builder} {}
  ...
  std::vector<JsonValue> Targets(const LabelTargetVector& targets,
  ...
  JsonValue StrictDepsTargets() {
    ...
    const auto& default_toolchain =
        targets.front()->settings()->default_toolchain_label();
    const auto& build_root = build_settings_->build_dir();

    out.reserve(targets.size());
    for (const auto& target : targets) {
      base::flat_map<std::string, std::unique_ptr<JsonValue>> cur;
      cur["name"] = std::make_unique<JsonValue>(
          Compress(target->label().GetUserVisibleName(default_toolchain)));
      cur["deps"] = std::make_unique<JsonValue>(
          Targets(target->private_deps(), default_toolchain));
      cur["public_deps"] = std::make_unique<JsonValue>(
          Targets(target->public_deps(), default_toolchain));
      cur["public_headers"] =
          std::make_unique<JsonValue>(Files(target->public_headers()));
      ...
      for (const auto& source : sources) {
        ...
        if (target->GetOutputFilesForSource(source, &tool, &output_files)) {
          if (!output_files.empty()) {
            ...
            auto source_number =
                Compress(OutputFile(build_settings_, source).value()).GetInt();
            for (const auto& output_file : output_files)
              outputs.emplace_back(Compress(output_file.value()));
            // JsonWriter doesn't handle int keys for dictionaries, and I don't
            // feel like going to the effort of supporting it.
            source_outputs[std::to_string(source_number)] =
                std::make_unique<JsonValue>(std::move(outputs));
          }
        }
      }
      if (!source_outputs.empty()) {
        cur["source_outputs"] =
            std::make_unique<JsonValue>(std::move(source_outputs));
      }
...
bool WriteStrictDeps(const BuildSettings* settings,
  ...
  values["targets"] =
      std::make_unique<JsonValue>(json_builder.StrictDepsTargets());
  values["strings"] =
      std::make_unique<JsonValue>(json_builder.CompressedStrings());
