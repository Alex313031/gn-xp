...
TEST_F(JSONWriter, ActionWithResponseFile) {
  ...
  base::FilePath root_path = base::FilePath("/path/to/src");
  setup.build_settings()->SetRootPath(root_path);
  g_scheduler->AddGenDependency(root_path.Append(".gn"));
  g_scheduler->AddGenDependency(root_path.Append("BUILD.gn"));
  g_scheduler->AddGenDependency(root_path.Append("build/BUILD.gn"));
  std::string out =
