...
std::optional<Version> GetNinjaVersion(const base::FilePath& ninja_executable) {
  base::CommandLine cmdline(ninja_executable);
  cmdline.SetParseSwitches(false);
  cmdline.AppendSwitch("version");
  std::string output;
  Err err;

  // Ignore any errors if they occur. It is possible that ninja might not be
  // available, which is okay. This is a best effort approach.
  bool success = RunNinja(cmdline, base::FilePath("."), &output, &err);

  return success ? Version::FromString(std::string(
                       base::TrimWhitespaceASCII(output, base::TRIM_TRAILING)))
                 : std::nullopt;
}
