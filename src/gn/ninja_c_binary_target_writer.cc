...
void NinjaCBinaryTargetWriter::WriteCompilerVars() {
  ...
  if (subst.used.count(&CSubstitutionFrameworkDirs)) {
    ...
    RecursiveTargetConfigToStream<SourceDir>(
        ...
        FrameworkDirsWriter(framework_dirs_output), out_);
    out_ << std::endl;
  }

  // Frameworks.
  if (subst.used.count(&CSubstitutionFrameworks)) {
    out_ << CSubstitutionFrameworks.ninja_name << " =";
    RecursiveTargetConfigToStream<std::string>(
        target_, &ConfigValues::frameworks, FrameworksWriter(), out_);
    out_ << std::endl;
  }
