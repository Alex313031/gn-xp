...
bool CleanOneDir(const std::string& dir) {
  ...
  // Replace existing build.ninja and build.ninja.d with just enough for ninja
  // to call GN and regenerate ninja files.
  if (!commands::PrepareForRegeneration(&setup->build_settings())) {
    return false;
  }

  // Erase everything but these three files.
  const base::FilePath::CharType* remaining[]{
      FILE_PATH_LITERAL("args.gn"), FILE_PATH_LITERAL("build.ninja"),
