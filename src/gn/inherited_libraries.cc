...
InheritedLibraries::GetOrderedAndPublicFlag() const {
  ...
  for (size_t nn = 0; nn < targets_.size(); ++nn)
    result.emplace_back(targets_[nn], public_flags_[nn]);
  return result;
}

void InheritedLibraries::Append(const Target* target, bool is_public) {
  // Try to insert a new node.
  if (targets_.push_back(target)) {
    // New target was added, append public flag.
    public_flags_.push_back(is_public);
  } else if (is_public) {
    // Target already present, if |is_public|, set its flag.
    public_flags_[targets_.IndexOf(target)] = true;
  }
}
...
void InheritedLibraries::AppendInherited(const InheritedLibraries& other,
  ...
  for (size_t nn = 0; nn < other.targets_.size(); ++nn) {
    Append(other.targets_[nn], is_public && other.public_flags_[nn]);
  }
}
...
void InheritedLibraries::AppendPublicSharedLibraries(
    ...
    bool is_public) {
  for (size_t nn = 0; nn < other.targets_.size(); ++nn) {
    const Target* target = other.targets_[nn];
    if (target->output_type() == Target::SHARED_LIBRARY &&
        other.public_flags_[nn]) {
