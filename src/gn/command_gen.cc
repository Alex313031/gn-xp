...
int RunGen(const std::vector<std::string>& args) {
  ...
  auto strictDepsEnabled = setup->build_settings().build_args().GetArgOverride(variables::kStrictDeps);
  if (strictDepsEnabled != nullptr) {
    if (!strictDepsEnabled->VerifyTypeIs(Value::BOOLEAN, &err)) {
      err.PrintToStdout();
      return 1;
    }
    if (strictDepsEnabled->boolean_value() && !WriteStrictDeps(&setup->build_settings(), setup->builder(), &err)) {
      err.PrintToStdout();
      return 1;
    }
  } 
