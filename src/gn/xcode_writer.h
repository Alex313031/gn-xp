...
class XcodeWriter {
 ...
 private:
  XcodeWriter(const BuildSettings* build_settings, Options options);
  ~XcodeWriter();

  bool AddSourcesFromBuilder(const Builder& builder, Err* err);
  bool AddTargetsFromBuilder(const Builder& builder, Err* err);

  bool WriteFiles(Err* err) const;

  std::unique_ptr<XcodeWorkspace> workspace_;
  std::unique_ptr<XcodeProject> products_;
