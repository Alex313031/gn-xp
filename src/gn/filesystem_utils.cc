...
#if defined(OS_WIN)
std::string GetPathWithDriveLetter(const std::string_view& path) {
...
std::string RegulatePathIfAbsolute(const std::string_view& path) {
  // 1. /C:/ -> C:/
...
std::string MakeRelativePath(const std::string& input_raw,
  ...
  if (IsPathAbsolute(input) && IsPathAbsolute(dest) && input.size() > 1 &&
    ...
    if (input[0] != dest[0] &&
      ...
      input[0] = dest[0];
      return MakeRelativePath(input, dest);
    }
  }
