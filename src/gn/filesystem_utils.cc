...
OutputFile GetSubBuildDirAsOutputFile(const BuildDirContext& context,
  ...
  if (source_dir.is_source_absolute()) {
    ...
    if (source_dir_path.substr(0, build_dir.size()) == build_dir) {
      ...
      // sources. In this case, remove the build directory prefix, but nothing
      // more. Note that this will create results like `obj/gen` or
      // `toolchain2/obj/toolchain1/gen` which look surprising, but guarantee
      // unicity.
