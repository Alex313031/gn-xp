...
class SourceFile {
 public:
  ...
  explicit SourceFile(UniqueKey value);
  ...
  struct FastCompare {
    bool operator()(const SourceFile& a, const SourceFile& b) const noexcept {
      return UniqueKey::FastCompare()(a.value_, b.value_);
    }
  };
  struct FastHash {
    size_t operator()(const SourceFile& s) const noexcept {
      return UniqueKey::FastHash()(s.value_);
    }
  };

  struct FastEqual {
    bool operator()(const SourceFile& a, const SourceFile& b) const noexcept {
      return UniqueKey::FastEqual()(a.value_, b.value_);
    }
  };
...
using SourceFileSet = base::flat_set<SourceFile, SourceFile::FastCompare>;
