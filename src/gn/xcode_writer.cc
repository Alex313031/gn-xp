...
bool XcodeProject::AddSourcesFromBuilder(const Builder& builder, Err* err) {
  ...
  auto TryToAddFileForIndexing = [&](std::string& target_name, const SourceFile& source){
    if (ShouldIncludeFileInProject(source)) {
      ...
      project_.AddSourceFileToTargetForIndexing(target_name, source_file, source_file, CompilerFlags::NONE);
    }
  };
...
std::vector<std::string> GetIncludeDirs(const Target* target, const BuildSettings* build_settings_) {
  ...
  for (ConfigValuesIterator it(target); !it.done(); it.Next()) {
     for (const SourceDir& include_dir : it.cur().include_dirs()) {
       paths.push_back(build_settings_->GetFullPath(include_dir).value());
     }
  }
...
PBXNativeTarget* XcodeProject::AddBinaryTarget(const Target* target,
  ...
  return project_.AddNativeTarget(
      ...
      GetBuildScript(target->label().name(), options_.ninja_executable,
                     options_.ninja_extra_args, env), PBXAttributes(), GetIncludeDirs(target, build_settings_));
}
