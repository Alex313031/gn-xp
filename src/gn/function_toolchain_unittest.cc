...
TEST_F(FunctionToolchain, RustRuntimeOutputs) {
  ...
  // These runtime outputs are a subset of the outputs so are OK.
  {
    TestParseInput input(
        R"(toolchain("rust") {
          tool("rust_cdylib") {
            command = "{{rustenv}} rustc --crate-name {{crate_name}} --crate-type cdylib {{rustflags}} -o {{output}} {{externs}} {{source}}"
            description = "RUST {{output}}"
            outputs = [ "foo" ]
            runtime_outputs = [ "foo" ]
          }
    ...
    const Tool* rust = toolchain->GetTool(RustTool::kRsToolCDylib);
    ASSERT_TRUE(rust);
    ASSERT_EQ(
        rust->command().AsString(),
        "{{rustenv}} rustc --crate-name {{crate_name}} --crate-type cdylib "
        "{{rustflags}} -o {{output}} {{externs}} {{source}}");
    ASSERT_EQ(rust->description().AsString(), "RUST {{output}}");
  }
}
