...
Tool variables
    ...
    module_switch [string, optional, link tools only]
        Valid for: Linker tools except "alink"
        ...
        If you specified:
          module_swift = "-Wl,-add_ast_path,"
        then the "{{modules}}" expansion for
    ...
    partial_outputs  [list of strings with substitutions]
        ...
        This is used to deal with whole module optimisation, allowing to
        list one object file per source file when whole module optimisation
        is disabled.
  ...
  expansions are available:
    ...
    {{modules}}
        ...
        files directly). Those will be prefixed with "module_switch" value.
  ...
  in addition to other type of ouputs. The following expansions are available:
    ...
    {{module_dirs}}
        Expands to the list of -I<path> for the target module search path
        computed from target dependencies.
