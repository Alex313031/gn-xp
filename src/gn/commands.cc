...
template <class Type>
auto GetItemAs(const Item* item) {
  using ItemType = std::decay_t<std::remove_pointer_t<Type>>;
...
// Filter a container of Item pointers into a container of
// Target, Config or Toolchain pointers
template <class Inputs, class Outputs>
void FilterItems(Inputs&& inputs, Outputs& outputs) {
  using ValueType = typename Outputs::value_type;

  for (auto input : inputs) {
    if (auto item = GetItemAs<ValueType>(input)) {
...
bool LabelQuery::ResolveFromCommandLineInput(const std::string& input,
  ...
  } else if (auto item = ResolveItem(setup_, current_dir_, input)) {
...
bool LabelQuery::ResolveFromCommandLineInput(
  ...
  for (const auto& cur : input) {
    if (!ResolveFromCommandLineInput(cur, default_toolchain_only,
                                     item_type)) {
      return false;
    }
  }
...
int LabelQuery::GetItemType() {
  auto cmdline = base::CommandLine::ForCurrentProcess();
  auto value = cmdline->GetSwitchValueASCII("type");
