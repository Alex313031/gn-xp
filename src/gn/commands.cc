...
// The BOOL, BOOL_EXPR, ENUM and STRING argument should be macros
// with the following signatures:
//
//   BOOL(name, switch):
//     For a boolean flag, stored in CommandSwitches class member
//     |name|, using the command-line |switch| which must be a string.
//
//   BOOL_ENUM(name, switch, type):
//     For a boolean flag obtained by parsing the |switch| value
//     through a |type|-specific parser.
//
//   ENUM(name, switch, type):
//     For an enum value of a given |type|, obtained by parsing the
//     |switch| value.
//
//   STRING(name, switch):
//     For a string value that comes directly from the |switch| value.
//
// The |parse| callable should have a signature that looks like:
//
//    bool Parse(const std::string& value, TYPE* member);
//
// On success, it shall return true and set |*member|. On failure,
// which means an invalid value, it shall return false after printing
// an error message.
//
...
// Tag type only used for selecting the right CmdTraits
// specialization.
...
struct CmdTraits<FormatJsonBool> : public CmdTraits<bool> {
  ...
  static bool Parse(const base::CommandLine& cmdline,
                    const char* name,
                    Type* member) {
    *member = cmdline.GetSwitchValueASCII(name) == "format";
    return true;
  }
};
...
struct CmdTraits<TargetPrintMode> : public CmdEnumTraits<TargetPrintMode> {
  ...
  static bool Parse(const base::CommandLine& cmdline,
    ...
    if (!cmdline.HasSwitch(name))
      return true;
    std::string value = cmdline.GetSwitchValueASCII(name);
    if (value == "buildfile") {
...
struct CmdTraits<std::string> {
  ...
  static bool Parse(const base::CommandLine& cmdline,
                    const char* name,
                    Type* member) {
    *member = cmdline.GetSwitchValueASCII(name);
    return true;
  }
