...
void NinjaBinaryTargetWriter::WriteSourceSetStamp(
  ...
  UniqueVector<OutputFile> extra_object_or_rs_files;
  UniqueVector<const Target*> linkable_deps;
  UniqueVector<const Target*> non_linkable_deps;
  UniqueVector<const Target*> framework_deps;
  GetDeps(&extra_object_or_rs_files, &linkable_deps, &non_linkable_deps,
          &framework_deps, &extra_object_or_rs_files);
  ...
  // instead.
  DCHECK(extra_object_or_rs_files.empty());
...
void NinjaBinaryTargetWriter::GetDeps(
  ...
  for (const auto& pair : target_->GetDeps(Target::DEPS_LINKED)) {
    ClassifyDependency(pair.ptr, extra_object_files, linkable_deps,
                       non_linkable_deps, framework_deps, extra_rs_files);
  }
...
void NinjaBinaryTargetWriter::ClassifyDependency(
  ...
  if (dep->output_type() == Target::SOURCE_SET ||
    ...
    if (can_link_libs)
      AddSourceSetFiles(dep, extra_object_files, extra_rs_files);
...
void NinjaBinaryTargetWriter::AddSourceSetFiles(
  ...
  // If the source set contains any Rust files, do not pretend we know
  // how to make them into object files, but just let the caller know
  // we found them by putting them into |rs_files|.
  if (source_set->source_types_used().RustSourceUsed()) {
    for (const auto& source : source_set->sources()) {
      if (source.type() == SourceFile::SOURCE_RS) {
        rs_files->push_back(OutputFile(settings_->build_settings(), source));
      }
    }
  }
