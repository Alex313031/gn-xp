  ...
  for (size_t i = 0; i < all_libs.size(); i++) {
    ...
    if (base::EndsWith(lib_value, framework_ending,
      ...
      if (lib_file.is_source_file()) {
        ...
        path_output_.WriteDir(out, lib_file.source_file().GetDir(), PathOutput::DIR_NO_LAST_SLASH);
        framework_name = lib_file.source_file().GetName();
      }

      out << " " << tool->framework_switch();
      EscapeStringToStream(
          out, framework_name.substr(0, framework_name.size() - framework_ending.size()),
          lib_escape_opts);
