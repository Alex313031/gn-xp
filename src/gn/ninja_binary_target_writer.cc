...
NinjaBinaryTargetWriter::GetClassifiedDeps() const {
  ...
  // Inherited libraries.
  for (const auto& inherited : target_->inherited_libraries()) {
...
void NinjaBinaryTargetWriter::WriteLibrarySearchPath(
  ...
  const std::vector<SourceDir>& all_lib_dirs = target_->all_lib_dirs();
...
void NinjaBinaryTargetWriter::WriteLibs(std::ostream& out, const Tool* tool) {
  ...
  const std::vector<LibFile>& all_libs = target_->all_libs();
