...
MockInputFileManager::MockInputFileManager() {
  g_scheduler->input_file_manager()->load_file_callback_ = GetSyncCallback();
}

MockInputFileManager::~MockInputFileManager() {
  g_scheduler->input_file_manager()->load_file_callback_ = nullptr;
}
...
TEST_F(LoaderTest, TemplateBuildDependencyFilesAreCollected) {
  ...
  mock_ifm_.AddCannedResponse(
      SourceFile("//test.gni"),
      "template(\"tmpl\") {\n"
      "  executable(target_name) { sources = [ \"a.cc\" ] }\n"
      "}\n");
