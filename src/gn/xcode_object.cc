...
PBXTarget::PBXTarget(const std::string& name,
    ...
    : configurations_(
          std::make_unique<XCConfigurationList>(config_name, attributes, this, include_path)),
      name_(name) {
...
PBXProject::PBXProject(const std::string& name,
  ...
  configurations_ =
			std::make_unique<XCConfigurationList>(config_name, attributes, this, std::vector<std::string>());
}
...
void XCBuildConfiguration::Print(std::ostream& out, unsigned indent) const {
  ...
  IndentRules sub_rule{false, rules.level + 1};
