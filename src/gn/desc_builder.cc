...
#undef CONFIG_VALUE_ARRAY_HANDLER
    ...
    if (what(variables::kLibs)) {
      const auto& all_libs = resolved.all_libs(target_);
      if (!all_libs.empty()) {
    ...
    if (what(variables::kLibDirs)) {
      const auto& all_lib_dirs = resolved.all_lib_dirs(target_);
    ...
    if (what(variables::kFrameworks)) {
      const auto& all_frameworks = resolved.all_frameworks(target_);
    ...
    if (what(variables::kWeakFrameworks)) {
      const auto& weak_frameworks = resolved.all_weak_frameworks(target_);
    ...
    if (what(variables::kFrameworkDirs)) {
      const auto& all_framework_dirs = resolved.all_framework_dirs(target_);
