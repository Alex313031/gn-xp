...
const char kForwardVariablesFrom_Help[] =
  ...
  forward_variables_from will always overwrite variables in the current
  (destination) scope if it forwards an overlapping one from the given scope.
  This makes it functionally very similar to mirroring the variable in the
  current scope if the variable is defined in the given scope (see the first
  example below), with the notable exception that with a variable_list value of
  "*" does not copy from enclosing scopes.
