...
class NinjaBuildWriter {
 ...
 private:
  ...
  //
  // The workaround involves lying to ninja about when the build.ninja file is
  ...
  // needed, but does not list "build.ninja" as an output. The second statement
  // is named "build.ninja", but simply uses the phony rule to refer to the
  // first statement.
