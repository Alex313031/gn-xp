...
class CompileCommandsTest : public TestWithScheduler {
 public:
  ...
  // Returns a list of targets with the following hierarchy:
  //
  //   //foo:bar1
  //     //base:base
  //   //foo:bar2
  ...
  //   //random:random
  //
  // This returns a vector of raw pointers which is what the tested code
  // expects. To manage the lifetime of these pointers, the given |owning|
  // vector is passed in.
  std::vector<const Target*> GetTestTargets(
      std::vector<std::unique_ptr<Target>>* owning);

  // Returns true if the two target vectors contain the same targets, order
  // independent.
  bool VectorsEqual(const std::vector<const Target*>& a,
                    const std::vector<const Target*>& b) const {
    auto a_sorted = a;
    std::sort(a_sorted.begin(), a_sorted.end(), &CompareLabel);

    auto b_sorted = b;
    std::sort(b_sorted.begin(), b_sorted.end(), &CompareLabel);
...
std::vector<const Target*> CompileCommandsTest::GetTestTargets(
    std::vector<std::unique_ptr<Target>>* owning) {
  Err err;
  std::vector<const Target*> targets;

  auto icu_target = std::make_unique<Target>(
      settings(), Label(SourceDir("//third_party/"), "icu"));
  icu_target->set_output_type(Target::SOURCE_SET);
  icu_target->visibility().SetPublic();
  icu_target->SetToolchain(toolchain());
  EXPECT_TRUE(icu_target->OnResolved(&err));
  targets.push_back(icu_target.get());

  auto base_target =
      std::make_unique<Target>(settings(), Label(SourceDir("//base/"), "base"));
  base_target->set_output_type(Target::SOURCE_SET);
  base_target->visibility().SetPublic();
  base_target->SetToolchain(toolchain());
  EXPECT_TRUE(base_target->OnResolved(&err));
  targets.push_back(base_target.get());

  auto base_i18n =
      std::make_unique<Target>(settings(), Label(SourceDir("//base/"), "i18n"));
  base_i18n->set_output_type(Target::SOURCE_SET);
  base_i18n->visibility().SetPublic();
  base_i18n->private_deps().push_back(LabelTargetPair(icu_target.get()));
  base_i18n->public_deps().push_back(LabelTargetPair(base_target.get()));
  base_i18n->SetToolchain(toolchain());
  EXPECT_TRUE(base_i18n->OnResolved(&err))
      << err.message() << " " << err.help_text();
  targets.push_back(base_i18n.get());

  auto target1 =
      std::make_unique<Target>(settings(), Label(SourceDir("//foo/"), "bar1"));
  target1->set_output_type(Target::SOURCE_SET);
  target1->public_deps().push_back(LabelTargetPair(base_target.get()));
  target1->SetToolchain(toolchain());
  EXPECT_TRUE(target1->OnResolved(&err));
  targets.push_back(target1.get());

  auto target2 =
      std::make_unique<Target>(settings(), Label(SourceDir("//foo/"), "bar2"));
  target2->set_output_type(Target::SOURCE_SET);
  target2->public_deps().push_back(LabelTargetPair(base_i18n.get()));
  target2->SetToolchain(toolchain());
  EXPECT_TRUE(target2->OnResolved(&err));
  targets.push_back(target2.get());

  auto random_target = std::make_unique<Target>(
      settings(), Label(SourceDir("//random/"), "random"));
  random_target->set_output_type(Target::SOURCE_SET);
  random_target->SetToolchain(toolchain());
  EXPECT_TRUE(random_target->OnResolved(&err));
  targets.push_back(random_target.get());

  owning->push_back(std::move(base_target));
  owning->push_back(std::move(base_i18n));
  owning->push_back(std::move(icu_target));
  owning->push_back(std::move(target1));
  owning->push_back(std::move(target2));
  owning->push_back(std::move(random_target));
  return targets;
}
