...
// dependency, or part of a chain of public_deps from a direct dependency.
// Private transitive dependencies also must appear but are pointed to by
// -Ldependency as they are not available for use from the target crate. In the
// future they will move to --extern priv: when explicit private dependencies
// are stabilized.
TEST_F(NinjaRustBinaryTargetWriterTest, RlibDeps) {
  ...
  Target far_public_rlib(setup.settings(), Label(SourceDir("//far/"), "farlib"));
...
TEST_F(NinjaRustBinaryTargetWriterTest, RenamedDeps) {
  ...
  SourceFile transitive_lib("//foo/transitive/lib.rs");
  transitive.sources().push_back(SourceFile("//foo/transitive/transitive.rs"));
  ...
  Target rlib(setup.settings(), Label(SourceDir("//foo/"), "mylib"));
  rlib.set_output_type(Target::RUST_LIBRARY);
  rlib.visibility().SetPublic();
  SourceFile barlib("//foo/bar/lib.rs");
  rlib.sources().push_back(SourceFile("//foo/bar/mylib.rs"));
  rlib.sources().push_back(barlib);
  rlib.source_types_used().Set(SourceFile::SOURCE_RS);
  rlib.rust_values().set_crate_root(barlib);
  rlib.rust_values().crate_name() = "mylib";
  ...
  Target direct(setup.settings(), Label(SourceDir("//foo/"), "direct"));
  direct.set_output_type(Target::RUST_LIBRARY);
  direct.visibility().SetPublic();
  SourceFile direct_lib("//foo/direct/lib.rs");
  direct.sources().push_back(SourceFile("//foo/direct/direct.rs"));
...

  {
    ...
    const char expected[] =
        ...
        "\n"
        "build ./foo_bar: rust_bin ../../foo/main.rs | ../../foo/source.rs "
        "../../foo/main.rs obj/foo/libdirect.rlib obj/foo/libmylib.rlib\n"
        "  externs = --extern direct_renamed=obj/foo/libdirect.rlib "
        "--extern mylib=obj/foo/libmylib.rlib "
        "--extern transitive_renamed=obj/foo/libtransitive.rlib\n"
        "  rustdeps =\n"
        "  ldflags =\n"
        "  sources = ../../foo/source.rs ../../foo/main.rs\n";
    std::string out_str = out.str();
...
TEST_F(NinjaRustBinaryTargetWriterTest, TransitivePublicNonRustDeps) {
...

  {
    ...
    const char expected[] =
        ...
        "  externs = --extern mylib=obj/bar/libmylib.rlib\n"
        "  rustdeps = -Lnative=. -Clink-arg=-Bdynamic "
