...
class ResolvedTargetData {
 public:
  ...
  const ResolvedTargetDeps& GetDeps(const Target* target) const {
  ...
  const std::vector<const Target*>& GetHardDeps(const Target* target) const {
 ...
 private:
  ...
  // Helper data structure and funciton used by ComputeRustLibs().
