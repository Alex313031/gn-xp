...
OutputStream& OutputStream::operator<<(long long value) {
  ...
  if (has_sign) {
    // NOTE: |LLONG_MIN == -LLONG_MIN| so value is not necessarily
    // positive afterhits statement. However the loop below will
    // still generate the right sequence of digits, as long as
    // the stop condition if |value != 0|.
    value = -value;
  }
