...
#undef TAG3
  ...
  // Other cases
  auto ends_with_ext =
      [s = std::string_view(str, size)](std::string_view ext) -> bool {
    return EndsWithExtension(s, ext);
  };

  if (ends_with_ext("swift"))
    return SourceFile::SOURCE_SWIFT;

  if (ends_with_ext("swiftmodule"))
    return SourceFile::SOURCE_SWIFTMODULE;

  if (ends_with_ext("modulemap"))
