...
class NestedSet {
  ...
  struct Header {
    ...
    // Don't use std::max() here to avoid include <algorihtm> which is massive.
    static const size_t kAlignment =
        (alignof(T) > alignof(Header*)) ? alignof(T) : alignof(Header*);
    ...
    static Header* Allocate(size_t num_deps, size_t num_items) {
      ...
      Header* result =
          reinterpret_cast<Header*>(Allocator::Alloc(header.allocation_size()));
