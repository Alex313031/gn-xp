...
base::FilePath FindWindowsPython(const char16_t* pythonExeName,
                                 const char16_t* pythonBatName) {
  ...
  base::FilePath cur_dir_candidate_exe =
      base::FilePath(current_directory).Append(pythonExeName);
...
bool Setup::FillPythonPath(const base::CommandLine& cmdline, Err* err) {
  ...
  if (cmdline.HasSwitch(switches::kScriptExecutable)) {
    ...
    if (!script_executable.IsAbsolute() &&
      ...
      script_executable = FindWindowsPython(
          script_executable.ReplaceExtension(u".exe").value().c_str(),
          script_executable.ReplaceExtension(u".bat").value().c_str());
    } else {
    ...
    base::FilePath python_path =
        FindWindowsPython(kPythonExeName, kPythonBatName);
