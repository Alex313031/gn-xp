...
const char kAll[] = "all";
const char kUnresolved[] = "unresolved";

auto GetDescriptionFlags() {
  auto cmdline = base::CommandLine::ForCurrentProcess();
  return (cmdline->HasSwitch(kAll) ? DescBuilder::ALL : 0u) |
         (cmdline->HasSwitch(kTree) ? DescBuilder::TREE : 0u) |
         (cmdline->HasSwitch(kBlame) ? DescBuilder::BLAME : 0u) |
         (cmdline->HasSwitch(kUnresolved) ? DescBuilder::UNRESOLVED : 0u);
}
...
bool PrintTarget(const Target* target,
                 ...
                 bool single_target,
                 const std::map<std::string, DescHandlerFunc>& handler_map,
                 DescBuilder::Flags flags) {
  std::unique_ptr<base::DictionaryValue> dict =
      DescBuilder::DescriptionForTarget(target, what, flags);
...
bool PrintConfig(const Config* config,
                 ...
                 DescBuilder::Flags flags) {
  std::unique_ptr<base::DictionaryValue> dict =
      DescBuilder::DescriptionForConfig(config, what, flags);
  if (!what.empty() && dict->empty()) {
...
int RunDesc(const std::vector<std::string>& args) {
  ...
  auto default_toolchain_only = cmdline->HasSwitch(switches::kDefaultToolchain);
  auto flags = GetDescriptionFlags();

  // Deliberately leaked to avoid expensive process teardown.
  auto setup = new Setup;
  setup->builder().set_should_resolve(!(flags & DescBuilder::UNRESOLVED));
