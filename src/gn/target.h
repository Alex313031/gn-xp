...
class Target : public Item {
 public:
  ...
  Metadata& metadata();
  const Metadata* metadata_ptr() const { return metadata_.get(); }
  ...
  const GeneratedFile* generated_file_ptr() const {
    return generated_file_.get();
  }
  ...
  const BundleData* bundle_data_ptr() const { return bundle_data_.get(); }
  ...
  bool hard_dep() const {
    return output_type_ == ACTION || output_type_ == ACTION_FOREACH ||
           ...
           (IsBinary() && swift_values_ptr() && swift_values().builds_module());
  }
  ...
  const ConfigValues* config_values_ptr() const { return config_values_.get(); }

  ActionValues& action_values();
  const ActionValues& action_values() const;
  const ActionValues* action_values_ptr() const { return action_values_.get(); }

  SwiftValues& swift_values();
  const SwiftValues& swift_values() const;
  const SwiftValues* swift_values_ptr() const { return swift_values_.get(); }

  RustValues* rust_values_ptr() { return rust_values_.get(); }
  const RustValues* rust_values_ptr() const { return rust_values_.get(); }
