...
class Target : public Item {
 public:
  ...
  const UniqueVector<LabelConfigPair>& configs() const {
    return binary_data().configs_;
  }
  ...
  InheritedLibraries& rust_transitive_libs() {
  ...
  const UniqueVector<SourceDir>& all_lib_dirs() const {
    return binary_data().all_lib_dirs_;
  }
  const UniqueVector<LibFile>& all_libs() const {
    return binary_data().all_libs_;
  }
  ...
  const OutputFile& link_output_file() const {
    return binary_data().link_output_file_;
  }
