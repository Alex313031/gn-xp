...
int num_processors_in_group = GetActiveProcessorCount(processor_group);
int num_threads_allocated_to_group = 0;

int GetNextProcessorGroup() {
  if (num_threads_allocated_to_group >= num_processors_in_group) {
    // A thread has been allocated for every processor in the current group.
    // Begin allocating threads in the next group (or restart from group 0).
    processor_group++;
    if (processor_group >= kNumProcessorGroups) {
      processor_group = 0;
    }
    num_threads_allocated_to_group = 0;
    num_processors_in_group = GetActiveProcessorCount(processor_group);
  }
