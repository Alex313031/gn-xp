...
```
    ...
    depend_output  [string with substitutions]
        Valid for: "solink" or "rust_cdylib" only (optional)

        These two files specify which of the outputs from the solink or
        rust_cdylib tool should be used for linking and dependency tracking.
        These should match entries in the "outputs". If unspecified, the
        first item in the "outputs" array will be used for all. See
        "Separate linking and dependencies for shared libraries" below for
        more.
...
```
  ...
  expansions are available:
    ...
    {{solibs}}
        ...
        (if the solink and rust_cdylib tools specify a "link_output" variable
        separate from the "depend_output").
