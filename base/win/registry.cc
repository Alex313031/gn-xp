...
LONG RegKey::CreateWithDisposition(HKEY rootkey,
  ...
  HKEY subhkey = NULL;
  LONG result =
      RegCreateKeyEx(rootkey, ToWCharT(subkey), 0, NULL, REG_OPTION_NON_VOLATILE, access,
...
LONG RegKey::CreateKey(const char16_t* name, REGSAM access) {
  ...
  HKEY subkey = NULL;
  LONG result = RegCreateKeyEx(key_, ToWCharT(name), 0, NULL, REG_OPTION_NON_VOLATILE,
...
LONG RegKey::OpenKey(const char16_t* relative_key_name, REGSAM access) {
  ...
  HKEY subkey = NULL;
  LONG result = RegOpenKeyEx(key_, ToWCharT(relative_key_name), 0, access, &subkey);
...
bool RegKey::HasValue(const char16_t* name) const {
  return RegQueryValueEx(key_, ToWCharT(name), 0, NULL, NULL, NULL) == ERROR_SUCCESS;
}
...
LONG RegKey::GetValueNameAt(int index, std::u16string* name) const {
  ...
  LONG r = ::RegEnumValue(key_, index, ToWCharT(buf), &bufsize, NULL, NULL, NULL, NULL);
  if (r == ERROR_SUCCESS)
    *name = buf;
...
LONG RegKey::DeleteKey(const char16_t* name) {
  ...
  // behavior.
  LONG result =
      RegOpenKeyEx(key_, ToWCharT(name), 0, READ_CONTROL | wow64access_, &subkey);
  if (result != ERROR_SUCCESS)
    return result;
  RegCloseKey(subkey);
...
LONG RegKey::DeleteEmptyKey(const char16_t* name) {
  ...
  LONG result =
      RegOpenKeyEx(key_, ToWCharT(name), 0, KEY_READ | wow64access_, &target_key);
  ...
  if (result == ERROR_SUCCESS) {
    if (type == REG_SZ) {
      *out_value = raw_value;
      ...
      size = ExpandEnvironmentStrings(ToWCharT(raw_value), ToWCharT(expanded), kMaxStringLength);
...
LONG RegKey::WriteValue(const char16_t* name, const char16_t* in_value) {
  return WriteValue(
      name, in_value,
      static_cast<DWORD>(sizeof(*in_value) * (wcslen(ToWCharT(in_value)) + 1)), REG_SZ);
}
...
LONG RegKey::RegDeleteKeyExWrapper(HKEY hKey,
  ...
  if (reg_delete_key_ex_func)
    return reg_delete_key_ex_func(hKey, ToWCharT(lpSubKey), samDesired, Reserved);
...
LONG RegKey::RegDelRecurse(HKEY root_key,
  ...
  while (result == ERROR_SUCCESS) {
    ...
    result =
        RegEnumKeyEx(target_key, 0, ToWCharT(WriteInto(&key_name, kMaxKeyNameLength)),
...
void RegistryValueIterator::Initialize(HKEY root_key,
  ...
  LONG result =
      RegOpenKeyEx(root_key, ToWCharT(folder_key), 0, KEY_READ | wow64access, &key_);
  if (result != ERROR_SUCCESS) {
    key_ = NULL;
  } else {
    DWORD count = 0;
...
bool RegistryValueIterator::Read() {
  if (Valid()) {
    ...
    LONG result = ::RegEnumValue(
        key_, index_, ToWCharT(WriteInto(&name_, name_size)), &name_size, NULL, &type_,
...
bool RegistryKeyIterator::Read() {
  if (Valid()) {
    ...
    FILETIME written;
    LONG r = ::RegEnumKeyEx(key_, index_, ToWCharT(name_), &ncount, NULL, NULL, NULL,
                            &written);
    if (ERROR_SUCCESS == r)
      return true;
  }
