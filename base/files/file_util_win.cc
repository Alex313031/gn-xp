...
bool ReplaceFile(const FilePath& from_path,
  ...
  if (::ReplaceFile(ToWCharT(&to_path.value()), ToWCharT(&from_path.value()), NULL,
...
bool PathExists(const FilePath& path) {
  return (GetFileAttributes(ToWCharT(&path.value())) != INVALID_FILE_ATTRIBUTES);
}
...
bool CreateTemporaryFileInDir(const FilePath& dir, FilePath* temp_file) {
  ...
  DWORD long_name_len =
      GetLongPathName(ToWCharT(&temp_name.value()), ToWCharT(long_temp_name), MAX_PATH);
...
bool CreateDirectoryAndGetError(const FilePath& full_path, File::Error* error) {
  ...
  if (!::CreateDirectory(ToWCharT(&full_path.value()), NULL)) {
    ...
    if (error_code == ERROR_ALREADY_EXISTS && DirectoryExists(full_path)) {
      ...
      DLOG(WARNING) << "Failed to create directory " << UTF16ToUTF8(full_path.value())
...
bool DevicePathToDriveLetterPath(const FilePath& nt_device_path,
  ...
  if (!::GetLogicalDriveStrings(kDriveMappingSize - 1, ToWCharT(drive_mapping))) {
  ...
  while (*drive_map_ptr) {
    ...
    if (QueryDosDevice(ToWCharT(drive), ToWCharT(device_path_as_string), MAX_PATH)) {
...
bool NormalizeToNativeFilePath(const FilePath& path, FilePath* nt_path) {
  ...
  win::ScopedHandle file_handle(::CreateFile(ToWCharT(&path.value()), GENERIC_READ,
  ...
  if (::GetMappedFileNameW(cp, file_view, ToWCharT(mapped_file_path), kMaxPathLength)) {
...
int WriteFile(const FilePath& filename, const char* data, int size) {
  win::ScopedHandle file(CreateFile(ToWCharT(&filename.value()), GENERIC_WRITE, 0,
...
bool AppendToFile(const FilePath& filename, const char* data, int size) {
  win::ScopedHandle file(CreateFile(ToWCharT(&filename.value()), FILE_APPEND_DATA,
...
int GetMaximumPathComponentLength(const FilePath& path) {
  ...
  if (!GetVolumePathNameW(
          ToWCharT(&path.NormalizePathSeparators().value()),
          ToWCharT(volume_path), arraysize(volume_path))) {
    return -1;
  }

  DWORD max_length = 0;
  if (!GetVolumeInformationW(ToWCharT(volume_path), NULL, 0, NULL, &max_length, NULL,
                             NULL, 0)) {
    return -1;
  }
