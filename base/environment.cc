...
class EnvironmentImpl : public Environment {
 ...
 private:
...
#if defined(OS_WIN)
    DWORD value_length =
        ::GetEnvironmentVariable(reinterpret_cast<LPCWSTR>(UTF8ToUTF16(variable_name).c_str()), nullptr, 0);
    if (value_length == 0)
      return false;
    if (result) {
      std::unique_ptr<char16_t[]> value(new char16_t[value_length]);
      ::GetEnvironmentVariable(reinterpret_cast<LPCWSTR>(UTF8ToUTF16(variable_name).c_str()), value.get(),
                               value_length);
  ...
  bool SetVarImpl(StringPiece variable_name, const std::string& new_value) {
    ...
    return !!SetEnvironmentVariable(reinterpret_cast<LPCWSTR>(UTF8ToUTF16(variable_name).c_str()),
...
#if defined(OS_WIN)
    // On success, a nonzero value is returned.
    return !!SetEnvironmentVariable(reinterpret_cast<LPCWSTR>(UTF8ToUTF16(variable_name).c_str()), nullptr);
#elif defined(OS_POSIX) || defined(OS_FUCHSIA)
    // On success, zero is returned.
